...
- 착수 시점(2026-07-15) 기준 `Json/docs/Plan.md`와 `Json/docs/phase1-serialization.md`가
  이미 스키마를 확정한 상태였다: `data/samples.json`
  `{"samples":[{sampleId,name,avgProductionTime,yield,stock}]}`,
  `data/orders.json` `{"orders":[{orderId,sampleId,customerName,quantity,status,
  createdAt}]}`, `createdAt`은 ISO8601 `YYYY-MM-DDTHH:MM:SS` 문자열. Phase 0의
  가정 스키마와 완전히 일치해 별도 수정 없이 그대로 채택했다.
- `Json/`은 nlohmann.json 3.11.3(NuGet)을 사용하므로 Dummy도 동일 버전을
  `packages/nlohmann.json.3.11.3`로 설치해 동일 라이브러리로 맞췄다(코드 공유는
  하지 않고 `JsonIo.h/.cpp`에서 to/from json 변환을 다시 구현).
- `Json/`의 `Order.createdAt`은 `std::string`이지만, Dummy의 `DummyOrder.createdAt`은
  이미 Phase 2에서 `std::chrono::system_clock::time_point`로 확정되어 있었으므로,
  `JsonIo`에 `FormatIso8601`/`ParseIso8601` 변환 함수를 추가해 JSON 문자열과
  왕복하도록 했다(초 단위까지 보존, 밀리초 이하는 버림).

## 완료 현황

- Phase 4 대상 테스트 9개(JsonIoTest 8개 포함, 총 17개 신규) 모두 통과.
- `AppendDummySamples`/`AppendDummyOrders`는 기본값 `clearExisting=false`로
  append가 기본 동작이며, `true`로 넘길 때만 기존 데이터를 버린다(선택 테스트
  `AppendSamples_ClearOptionReplacesFileInsteadOfAppending`로 고정).
- 손상된 JSON 파일은 `DummyDataIoException`을 던지고 원본 파일 내용을
  그대로 보존한다(쓰기는 `.tmp` 파일에 먼저 쓴 뒤 `std::filesystem::rename`으로
  원자적으로 교체).
